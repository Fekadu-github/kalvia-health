...
class ProviderProfile(Base):
    ...
    # A 3:4 portrait photo, stored as JPEG bytes in the database so it
    # survives redeploys (Render's free tier has no persistent disk).
    # photo_version carries a fresh random value on every upload so
    ...
    def photo_url(self):
        ...
        pattern as jitsi_join_url — no migration needed if photos
        move to object storage later. The ?v= token busts the
        browser cache whenever the photo is replaced."""
        if self.photo_version:
